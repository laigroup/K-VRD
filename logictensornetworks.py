...
def disjunction_of_literals(literals, label="no_label"):
    ...
    if default_aggregator == "hmean":
        ...
        return tf.div(tf.to_float(tf.size(result)), tf.reduce_sum(tf.reciprocal(result), keepdims=True))
    ...
    if default_aggregator == "cmean":
        ...
        return tf.pow(tf.reduce_mean(tf.pow(result, 3), keep_dims=True), tf.reciprocal(tf.to_float(3)), name=label)
...
class Predicate:
    def __init__(self, label, domain, layers, defined=None, type_idx=None):
        self.label = label
...
class Literal:
    def __init__(self, polarity, predicate, domain):
...
class KnowledgeBase:
    ...
    def __init__(self, label, clauses, save_path=""):
        ...
        if not self.clauses:
            ...
            if default_clauses_aggregator == "whmean":
                print("clauses aggregator is whmean")
                weights_tensor = tf.constant([cl.weight for cl in clauses])
                self.tensor = tf.div(tf.to_float(tf.size(clauses_value_tensor)),
                                     tf.reduce_sum(tf.multiply(weights_tensor, tf.reciprocal(clauses_value_tensor)),
                                                   keep_dims=True))
                # self.tensor =tf.reduce_sum(tf.multiply(weights_tensor, tf.reciprocal(clauses_value_tensor)),
                #                                    keep_dims=True)
